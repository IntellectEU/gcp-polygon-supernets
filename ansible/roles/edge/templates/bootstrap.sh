...
{% if (hostvars[item].labels.role == "fullnode" or hostvars[item].labels.role == "validator") %}
    polygon-edge polybft-secrets --data-dir {{ hostvars[item].labels.node_name }} --json --insecure > {{ hostvars[item].labels.node_name }}.json
{% endif %}
{% endfor %}
...
{% if (hostvars[item].labels.role == "validator") %}
    ...
    polygon-edge polybft register-validator \
                 --data-dir {{ hostvars[item].labels.node_name }} \
                 --supernet-manager $(cat genesis.json | jq -r '.params.engine.polybft.bridge.customSupernetManagerAddr') \
                 --jsonrpc {{ rootchain_json_rpc }}
...
{% endfor %}
    ...
    tar czf {{ base_dn }}.tar.gz *.json validator* core-contracts
    popd
}
